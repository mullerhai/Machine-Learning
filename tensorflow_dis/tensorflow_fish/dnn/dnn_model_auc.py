...
flags.DEFINE_string("optimizer", None,
                    "Optimizer(adam,mom,adagrad,sgd)")
...
flags.DEFINE_integer("report_step", 100,
                     "Number of (global) training steps to perform")
flags.DEFINE_integer("eval_step", 10000,
                     "Number of (global) training steps to perform")
...
def get_optimizer():
    if FLAGS.optimizer == "adam":
      ...
      return tf.train.MomentumOptimizer(0.01, 0.85)
...
def model(input_data,ws,bs,train=False):
    ...
    for i in range(0,len(is_add_dropout)):
      ...
      #hid = tf.nn.elu(hid_lin) if i < len(is_add_dropout)-1 else hid_lin
      hid = tf.nn.relu(hid_lin) if i < len(is_add_dropout)-1 else hid_lin
      #hid = tf.tanh(hid_lin) if i < len(is_add_dropout)-1 else hid_lin
...
def train(logits, y_, global_step):
    ...
    opt = get_optimizer()
    log("Optimizer:" + str(opt))
    grads = opt.compute_gradients(cross_entropy)
    train_op = opt.apply_gradients(grads, global_step=global_step)
    return train_op, cross_entropy
    
device_setters = [tf.train.replica_device_setter(ps_tasks = FLAGS.num_parameter_servers,worker_device="/job:worker/task:%d" % i) for i in range(0,FLAGS.num_workers)]
def eval():
    eval_predictions = []
    eval_ys_ = []
    for device_setter in device_setters:
      with tf.device(device_setter):
        for i in xrange(FLAGS.eval_iter):
          #print(i)
          ...
          eval_ys_.append(eval_y_)
    with tf.device(device_setters[worker_index]):
	return tf.concat(0,eval_predictions),tf.concat(0,eval_ys_)
...
with sv.managed_session(master = master, config = sess_config) as sess:
    ...
    if is_chief:
      ...
      sv.loop(FLAGS.eval_secs, log_auc,(sess,eval_prediction, eval_y_, 1))
    avg_step = 0
    avg_loss = 0.0
    while not sv.should_stop():
      ...
      if step % FLAGS.report_step ==0:
        ...
        avg_step = 0
        avg_loss = 0.0
      #if is_chief and step % FLAGS.eval_step ==0:
      #  log("%f:AUC of step:%d" % (now, step))
      #  log_auc(sess, eval_prediction, eval_y_, 1)
        #log("%f: Worker %d: training step %d done (global step: %d),avg_step: %d, cross entropy = %g" % (now, worker_index, local_step, step,avg_step, avg_loss/avg_step/FLAGS.batch_size))
      if step >= train_steps:
        break
